function path_constraints  = generate_side_to_side_trajectory_constraints(start_point,           ...
    ...
    path_constraints = [path_constraints;...
        [start_point(1)+2*turn_radius, start_point(2)+sign*2*turn_radius,                    0,...
         0,                          sign*surge_velocity*(turn_velocity_percentage/100), 0,...
         sign*90,                    0,                                                  0]];
    path_constraints = [path_constraints;...
        [path_constraints(end,1),path_constraints(end,2)+sign*(line_distance/2-2*turn_radius),0,...
        0,                       sign*surge_velocity*(turn_velocity_percentage/100),      0,...
        sign*90,                 0,                                                       0]];
    ...
    for line_n = 1:number_of_lines
        if mod(line_n,2) == 0
            % Add turn
            next_turn_point         = path_constraints(end,1:3) + [2*turn_radius, 0, 0];
